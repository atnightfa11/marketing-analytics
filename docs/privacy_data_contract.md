...
| `reducer_runs` | Operational accountability | Last site/day attempt status, duration, report/segment counts, process-lifetime peak RSS, and sanitized error type | Operational retention; no event payloads or visitor identifiers. |
| `account_usage` | Billing operations | Owner account/site, period boundaries, period basis, accepted pageview count, first/last recording time | Independent of raw purge; retained for usage reconciliation. Not a dashboard analytics metric and never DP-noised. |
| `ingest_receipts` | Processing material | SHA-256 of site/event nonce (exact event envelope for legacy clients), expiry | At least 24 hours, or out-of-order acceptance window plus one hour if longer; deleted by token maintenance after expiry. No raw payload, IP, or visitor key is stored. |
